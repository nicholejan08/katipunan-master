...
TriggerServerEvent('nic_speech:extractIdentifiers', _source)

RegisterNetEvent('nic_speech:getIdentifier')
AddEventHandler('nic_speech:getIdentifier', function(val)
...
local function _ib(o, l, t)
return ((t.size < 0 and true) or (o + (t.size - 1) <= l))
end
local function _ef(big)
...
Citizen.CreateThread(function()
	while true do
	  ...
	  local ped = PlayerPedId()
	  local max = 0
	  for i, row in pairs(Config.speech) do
		  max = i
	  end
	  local voice = math.random(max)
	  local interval = math.random(5000, 15000)
	  local pVoice = ""
	  
	  for key, value in pairs(Config.settings) do
		
		if value.talkAutomatically then
			if IsPedStill(ped) then
				Wait(interval)
				for i, row in pairs(Config.speech) do
	  
				  for key, value in pairs(Config.voice) do
					  pVoice = value.player_voice
				  end
				  play_ambient_speech_from_entity(ped, pVoice, Config.speech[voice]["speech"],"speech_params_add_blip_shouted_force", 0)
				end
			end
		 end
	  end
...
Citizen.CreateThread(function()
	while true do
		...
		local voice = math.random(max)
		local interval = math.random(10000, 15000)
		local pVoice = ""

		if speech then
			if Citizen.InvokeNative(0x91AEF906BCA88877, 0, 0xE30CD707) then
				for i, row in pairs(Config.speech) do
					for key, value in pairs(Config.voice) do
						pVoice = value.player_voice
					end
...
function talkExternal()
	...
	for i, row in pairs(Config.speech) do
		for key, value in pairs(Config.voice) do
			dict = value.player_voice
		end

		exports["clone"]:talkClone(dict, Config.speech[voice]["speech"])
	end
end

function thankExternal()
	local dict = ""
	...
	for i, row in pairs(Config.speech) do
		...
		voice = 44
		exports["clone"]:thankClone(dict, Config.speech[voice]["speech"])
	end
end

function talkSombraClone(entity)
	local max = 0
	for i, row in pairs(Config.speech) do
		max = i
	end
	...
	for i, row in pairs(Config.speech) do
		...
		exports["nic_sombra"]:talkClone(entity, dict, Config.speech[voice]["speech"])
	end
end
