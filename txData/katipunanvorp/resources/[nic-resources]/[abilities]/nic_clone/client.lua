...
----------------------------------------------------------------------------------------------------
local clone, prop, peepee, randomItem, sack
local name = "Clone"
...
function talkClone(dict, message)
    if cloned then
        play_ambient_speech_from_entity(clone, dict, message,"speech_params_add_blip_shouted_force",0)
    end
end
...
function cloneSack()
    if DoesEntityExist(clone) then
        local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_Spine2")
        ...
        if DoesEntityExist(sack) then
            ...
            AttachEntityToEntity(sack, clone, boneIndex, -0.27, -0.41, -0.01, 335.0, 91.0, 0.0, true, true, false, true, 1, true)
        end
    end
end
...
function stopEffect()
	...
	Citizen.InvokeNative(0x523C79AEEFCC4A2A, clone, 10, "ALL")
    Citizen.Wait(200)
	ClearPedTasks(clone)
end
...
function playSmokeEffect()  
	...
	if not Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_particle_dict)) then -- HasNamedPtfxAssetLoaded
		...
		local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_L_Clavicle")

		smoke_particle_id = Citizen.InvokeNative(0x9C56621462FFE7A6, current_particle_name, clone, 0, 0, 0, -90.0, 0, 0, boneIndex, 0.4, 0, 0, 0)
...
function playSmoke2Effect()  
	...
	if not Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_particle_dict)) then -- HasNamedPtfxAssetLoaded
		...
		local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_Pelvis")

		smoke_particle2_id = Citizen.InvokeNative(0x9C56621462FFE7A6, current_particle_name, clone, 0, 0, 0, -90.0, 0, 0, boneIndex, 0.4, 0, 0, 0)
...
function playSmoke3Effect()  
	...
	if not Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_particle_dict)) then -- HasNamedPtfxAssetLoaded
		...
		local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_R_Thigh")
		
		smoke_particle3_id = Citizen.InvokeNative(0x9C56621462FFE7A6, current_particle_name, clone, 0, 0, 0, -90.0, 0, 0, boneIndex, 0.4, 0, 0, 0)
...
function playParticleEffect()
	...
	if not Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_particle_dict)) then -- HasNamedPtfxAssetLoaded
		...
		local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_Spine0")
		flies_particle_id = Citizen.InvokeNative(0x9C56621462FFE7A6, current_particle_name, clone, 0, 0, 0, -90.0, 0, 0, boneIndex, 1.0, 0, 0, 0)

		Citizen.InvokeNative(0x46DF918788CB093F, clone,"PD_Outhouse_Muck_Body_Face", 0.0, 0.0);  -- APPLY_PED_DAMAGE_PACK PD_Vomit for ped with damage 0.0 and mult 0.0
...
--                     TaskPlayAnim(clone, animation, "hog_ped_dead_vtm_pre_idl", 1.0, 8.0, -1, 31, 0, true, 0, false, 0, false)
--                 end
    
--                 if IsPedJumping(clone) or IsPedWalking(clone) or IsPedStopped(clone) or IsEntityInAir(clone) or IsPedFalling(clone) or IsPedOnMount(clone) or IsPedClimbing(clone) or cloneIsSharing and delivering then
--                     StopAnimTask(clone, animation, "hog_ped_dead_vtm_pre_idl", 1)
--                 end
--                 if IsPedRunning(clone) and not IsPedOnMount(clone) and not cloneIsSharing and not delivering then
--                     TaskPlayAnim(clone, animation, "hog_ped_dead_vtm_pre_idl", 1.0, 8.0, -1, 31, 0, true, 0, false, 0, false)
--                     TaskPlayAnim(clone, animation, "hog_ped_dead_vtm_pre_idl", 1.0, 8.0, -1, 31, 0, true, 0, false, 0, false)
--                 end
--                 if Citizen.InvokeNative(0x84D0BF2B21862059, PlayerPedId()) then
--                     Citizen.InvokeNative(0x7DE9692C6F64CFE8, clone, false, 2, false)
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_L_Hand")
        ...
        local cHolding = Citizen.InvokeNative(0xD806CD2A4F2C2996, clone)
        RequestAnimDict(animation)
        while ( not HasAnimDictLoaded(animation)) do 
            Citizen.Wait( 100 )
        end
        ...
        if cloneDrinking then            
            DisablePlayerFiring(clone, true)
        end
        if not IsPedInWrithe(clone) and IsPedHuman(clone) and DoesEntityExist(clone) and cloneHealth <= 40 and not IsPedRagdoll(clone) and not saving and not IsEntityDead(clone) then
            Citizen.Wait(3000)
            Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 2, true)
            cloneDrinkFirstaid()
        end
        if DoesEntityExist(clone) and playerHealth <= 40 and not shareItem and not IsPedRagdoll(clone) and not cloneDrinking then
            DetachEntity(cHolding, 1, 1)
            delivering = true
            TaskGoToEntity(clone, PlayerPedId(), 1.0, 1, 12.0, 1, 1)
            saving = true
            shareItem = true
            cloneIsSharing = true
        else
        ...
        if saving and not IsEntityDead(clone) and not cloneControl then
            for key, value in pairs(Config.settings) do
                if IsPedSprinting(clone) then
        ...
        if IsPlayerNearCloneWithFirstAid(cx, cy, cz) and saving and not IsPedRagdoll(clone) and playerHealth <= 40 then
            
            local cloneHealth = GetEntityHealth(clone)
            TaskPlayAnim(clone, animation2, "hold", 8.0, -8.0, -1, 31, 0, true, 0, false, 0, false)
            offered = true
            delivering = false
            cloneIsSharing = false
            ...
            SetEntityHealth(clone, 0, 0)
            Citizen.Wait(100)
            local qx, qy, qz = table.unpack(GetEntityCoords(clone, 0))
    ...
    while true do
        ...
        if DoesEntityExist(clone) then
            if IsControlJustPressed(0, 0x4BC9DABB) then
                ClearPedTasks(clone)
                Citizen.InvokeNative(0x5AD23D40115353AC, PlayerPedId(), clone, 1000, 1, 0, 0)
                Wait(1000)
                Citizen.InvokeNative(0xB31A277C1AC7B7FF, PlayerPedId(), 0, 0,901097731, 1, 1, 0, 0)
                TaskGoToEntity(clone, PlayerPedId(), 1.0, 1, 12.0, 1, 1)
                SetPedAsGroupMember(clone, GetPedGroupIndex(PlayerPedId()))
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        if DoesEntityExist(clone) and not IsEntityDead(clone) and not IsPedHuman(clone) then
            SetPlayerHealthRechargeMultiplier(clone, 2.0)
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if cloneDrinking and DoesEntityExist(clone) then
            DrawMedicine3D(cx, cy, cz+1.1)
        end
        if not IsCinematicCamRendering() and not IsPauseMenuActive() and IsPlayerNearCloneHealthRadius(cx, cy, cz) then
            if not IsEntityDead(clone) and not saving and not cloneControl then
                SetPlayerHealthRechargeMultiplier(clone, 0.0)
                for key, value in pairs(Config.settings) do
                    if IsPedOnMount(clone) and DoesEntityExist(clone) and IsPedHuman(clone) and not IsEntityDead(clone) then
                    ...
                    elseif DoesEntityExist(clone) and not IsEntityDead(clone) then
                        if IsPedHuman(clone) then
                            if IsPedOnMount(clone) then
                                cz = cz+0.8
                            end
                            if value.displayNametag then
                                if cloned then
                            ...
                            local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_Spine2")
                            local bCoords = GetWorldPositionOfEntityBone(clone, GetPedBoneIndex(clone, 0))

                            if value.displayNametag then
                                if cloned then
                                    DrawHeart3D(bCoords.x, bCoords.y, bCoords.z+1.0, "")
                                else
                                    DrawHeart3D(bCoords.x, bCoords.y, bCoords.z+1.0, value.nametag)
                                end
                            else
                                DrawHeart3D(bCoords.x, bCoords.y, bCoords.z+1.0, "")
                            end
            ...
            if IsEntityDead(clone) and not IsEntityDead(PlayerPedId()) and DoesEntityExist(clone) then
                DrawCloneDead3D(cx, cy, cz+1.1)
            end          

        else
            if not IsCinematicCamRendering() and not IsPauseMenuActive() then
                if IsEntityDead(clone) and not IsEntityDead(PlayerPedId()) and DoesEntityExist(clone) and not cloneControl then
                    DrawCloneDead3D(cx, cy, cz+1.1)
                else
                    if not saving then
...
CreateThread(function()
	while true do
		Wait(200)
        SetEntityAlpha(clone, GetEntityAlpha(PlayerPedId()), false)
    end
end)
...
CreateThread(function()
	while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            for key, value in pairs(Config.settings) do
                if value.associateType == "custom" and DoesEntityExist(clone) then
                    spawning = true
                    castSpell()
                    ...
                    DeleteEntity(clone)
                    spawning = false
                elseif value.associateType == "clone" and DoesEntityExist(clone) then
                    SetEntityHealth(clone, 0, 0)
                    breakingSpell = true
                    castSpell()
                end
...
CreateThread(function()
	while true do
		Wait(5)
        if not IsEntityDead(clone) or DoesEntityExist(clone) then
            SetPlayerHealthRechargeMultiplier(clone, 0.0)
        end
    end
end)
...
AddEventHandler('nic_clone:offerTimer', function()
	...
	Citizen.CreateThread(function()
		while timer > 0 and DoesEntityExist(clone) do
			Citizen.Wait(1000)
			timer = timer - 1
		end
        if timer == 0 then
            if DoesEntityExist(offeredProp) or IsEntityAttachedToEntity(offeredProp, clone) then
                playKeepkAnimation()
                requested = false
                timerStarted = false
            else
...
AddEventHandler('nic_clone:offerCooldown', function()
	...
	Citizen.CreateThread(function()
		while itemCooldown > 0 and DoesEntityExist(clone) do
			Citizen.Wait(1000)
...
--         if DoesEntityExist(clone) then
--             if IsPedInAnyVehicle(clone) then
--                 TaskVehicleDriveWander(clone, veh, 8.0, 1)
--                 print("TEST")
--             end
--         end
--     end
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        local cx, cy, cz = table.unpack(GetEntityCoords(clone))
        ...
        if not playingAnim and not IsPedSwimming(clone) and not Citizen.InvokeNative(0x3AA24CCC0D451379, clone) and not Citizen.InvokeNative(0xD453BB601D4A606E, clone) and not IsPedOnMount(PlayerPedId()) and not IsPedOnMount(clone) and IsPedHuman(clone) and not IsPedInAnyVehicle(clone, false) and not IsPedOnVehicle(clone, true) and not IsPedHangingOnToVehicle(clone) and not IsPedInFlyingVehicle(clone) and not IsPedSittingInAnyVehicle(clone) and not Citizen.InvokeNative(0xA911EE21EDF69DAF, clone) and not cHolding and not IsPedInMeleeCombat(PlayerPedId()) and not IsPedInMeleeCombat(clone) and not timerStarted and IsPlayerRequestItem(cx, cy, cz) and not requested and not requestCooldown and not cloneReacting and not cloneLaughing and not cloneDrinking and not IsEntityDead(clone) and DoesEntityExist(clone) and not IsPedRagdoll(clone) and cloneHealth >= 46 and IsPedFacingPed(PlayerPedId(), clone, 45.0) and IsPedFacingPed(clone, PlayerPedId(), 45.0) then
            DrawRequestItem3D(cx, cy, cz+1.3, "E", "Request Item")
            if IsControlJustPressed(0, 0xCEFD9220) and DoesEntityExist(clone) and not IsPedRagdoll(clone) then
                Citizen.InvokeNative(0x5AD23D40115353AC, PlayerPedId(), clone, 1000, 1, 0, 0)
                Citizen.InvokeNative(0x5AD23D40115353AC, clone, PlayerPedId(), 1000, 1, 0, 0)
                Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 2, true)
                ...
                Citizen.Wait(400)
                DeleteObject(prop)
                TaskPlayAnim(clone, dic, anim, 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
                
                offerRandomItem()
            end
        elseif not playingAnim and not Citizen.InvokeNative(0x3AA24CCC0D451379, clone) and not Citizen.InvokeNative(0xD453BB601D4A606E, clone) and IsPedHuman(clone) and Citizen.InvokeNative(0xA911EE21EDF69DAF, clone) and cHolding and not IsPedInMeleeCombat(PlayerPedId()) and not IsPedInMeleeCombat(clone) and not timerStarted and IsPlayerRequestItem(cx, cy, cz) and not requested and not requestCooldown and not cloneReacting and not cloneLaughing and not cloneDrinking and not IsEntityDead(clone) and DoesEntityExist(clone) and not IsPedRagdoll(clone) and cloneHealth >= 46 and IsPedFacingPed(PlayerPedId(), clone, 90.0) then
            DrawCarryItem3D(cx, cy, cz+1.3, "R", "Drop Carrying")
            if IsControlJustPressed(0, 0xE30CD707) and DoesEntityExist(clone) and not IsPedRagdoll(clone) then
                DetachEntity(cHolding, 1, 1)
                ClearPedTasks(clone)
            end
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if timerStarted and IsPlayerNearRandomItem(cx, cy, cz) and not IsEntityDead(clone) and DoesEntityExist(clone) and IsEntityPlayingAnim(clone, dic, anim, 31) and IsPedFacingPed(PlayerPedId(), clone, 45.0) and IsPedFacingPed(clone, PlayerPedId(), 45.0) then
            DrawRandomItem3D(cx, cy, cz+1.3, "E", "Get Item", offeredItem)

            if timerStarted and IsPlayerNearGetItem(cx, cy, cz) and IsControlJustPressed(0, 0xCEFD9220) then
                Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 2, true)
                ...
                Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -2121881035, 1, 1, 0, 0)
            end
...
function offerRandomItem()
    local cx, cy, cz = table.unpack(GetEntityCoords(clone))
    local boneIndex = GetEntityBoneIndexByName(clone, "SKEL_R_Hand")
    local max = 0
    ...
    if offeredItem == "banana" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.1, -0.11, -0.12, 259.0, 160.0, 6.0, true, true, false, true, 1, true)
    elseif offeredItem == "firstaid" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.09, -0.07, -0.08, 275.0, 0.0, 0.0, true, true, false, true, 1, true)
    elseif offeredItem == "firstaid-s" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.09, -0.07, -0.08, 275.0, 0.0, 0.0, true, true, false, true, 1, true)
    elseif offeredItem == "apple" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.08, 0.05, -0.08, 0.0, 0.0, 0.0, true, true, false, true, 1, true)
    elseif offeredItem == "pandesal" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.08, 0.05, -0.08, 0.0, 0.0, 0.0, true, true, false, true, 1, true)
    elseif offeredItem == "canteen" then
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.08, 0.08, -0.08, 296.0, 13.0, 4.0, true, true, false, true, 1, true)
    else
        AttachEntityToEntity(offeredProp, clone, boneIndex, 0.08, 0.05, -0.08, 0.0, 0.0, 0.0, true, true, false, true, 1, true)
    end
end
...
function playKeepkAnimation()
    ...
    TaskPlayAnim(clone, animation, "stn_long_low_skill_exit", 8.0, -1.0, -1, 31, 0, true, 0, false, 0, false)
    Wait(1200)
    DeleteObject(offeredProp)
    ClearPedTasks(clone)
end
    ...
    while true do
        ...
        if IsControlJustPressed(0, 0xCEE12B50) then
            ...
            if not carrying then
                ...
                if powerAcquired and not DoesEntityExist(clone) and not IsPedRagdoll(PlayerPedId()) then
                    TriggerServerEvent("nic_clone:usePower")
                elseif powerAcquired and DoesEntityExist(clone) and not IsPedRagdoll(PlayerPedId()) then
                    for key, value in pairs(Config.settings) do
                        if value.associateType == "custom" and DoesEntityExist(clone) then
                            spawning = true
                            Citizen.InvokeNative(0x5AD23D40115353AC, PlayerPedId(), clone, -1, 1, 1, 1)
                            castSpell()
                            kagebunshin(0.8)
                            playSmokeAudio()
                            Wait(200)
                            DeleteEntity(clone)
                            spawning = false
                        elseif value.associateType == "clone" and DoesEntityExist(clone) and not IsPedOnMount(clone) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedHuman(clone) and IsPedOnMount(PlayerPedId()) and not Citizen.InvokeNative(0x95CBC65780DE7EB1, clone, true) and DoesEntityExist(clone) and not IsPedRagdoll(clone) then
            cloneMount(playerMount)
        end
        
        if IsPedHuman(clone) and IsPedOnMount(PlayerPedId()) and DoesEntityExist(clone) then
            ClearPedTasks(clone)
        end
    end
end)
...
function cloneMount(playerMount)
    ClearPedTasks(clone)
    kagebunshin(1.0)
    Citizen.InvokeNative(0x028F76B6E78246EB, clone, playerMount, 0, true)
    playEffectAudio()
end
...
Citizen.CreateThread(function()
    while true do
        ...
        local cloneHeading = GetEntityHeading(clone)
        local pHealth = GetEntityHealth(PlayerPedId())
        local cHealth = GetEntityHealth(clone)
        ...
        if type and not cloneControl and not IsEntityPlayingAnim(PlayerPedId(), "mech_loco_m@generic@carry@ped@idle", "idle", 31) and DoesEntityExist(clone) and not IsPedInjured(clone) and not IsEntityDead(clone) and IsControlJustPressed(0, 0x26E9DC00) and not IsPedRagdoll(PlayerPedId()) and not IsPedRagdoll(clone) and not IsPedRagdoll(PlayerPedId()) and not IsPedOnMount(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsEntityInWater(PlayerPedId()) then
            transferring = true
            DetachEntity(carried, true, true)
            exports['progressbar']:startUI(1000, "Transferring Consciousness..")
            makeEntityFaceEntity(PlayerPedId(), clone)
            ...
            SetEntityHeading(clone, playerHeading)
            SetEntityCoords(PlayerPedId(), cloneCoords)
            SetEntityCoords(clone, playerCoords)
            SetEntityHealth(PlayerPedId(), cHealth, 0)
            SetEntityHealth(clone, pHealth, 0)
            TaskPlayAnim(clone, animation, "base", 8.0, -8.0, -1, 1, 0, true, 0, false, 0, false)
            TaskStartScenarioInPlace(clone, GetHashKey(animation2), -1, true, false, false, false)
            TriggerEvent("vorp:showUi", false)
            TriggerEvent("alpohud:toggleHud", false)
            TriggerEvent("vorp_hud:toggleHud", false)
        else
        ...
        if cloneControl then
            ...
            if not IsPlayerNearClone(cx, cy, cz) or IsEntityDead(clone) then
                dying = true
                tempsbar = 0
                cameraBars()
                AnimpostfxStop("PlayerDrugsPoisonWell")
                blip = SET_BLIP_TYPE(clone)
            ...
            if IsControlJustPressed(0, 0x26E9DC00) or pHealth <= 35 and not IsPedRagdoll(clone) and not IsPedRagdoll(PlayerPedId()) and not IsEntityInWater(PlayerPedId()) then
                ...
                FreezeEntityPosition(clone, false)
                TaskSetCrouchMovement(clone, false, 1, false)
                SetEntityHeading(PlayerPedId(), cloneHeading)
                SetEntityHeading(clone, playerHeading)
                SetEntityCoords(PlayerPedId(), cloneCoords)
                SetEntityCoords(clone, playerCoords)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5)
        if IsPedRagdoll(clone) then
            Citizen.InvokeNative(0xAE99FB955581844A, clone, -1, 0, 0, 0, 0, 0)
        end
    end
end)
    ...
    while true do
        ...
        for key, value in pairs(Config.settings) do
            if not IsEntityDead(PlayerPedId()) then
                ...
                if breakingSpell and not IsEntityDead(PlayerPedId()) and DoesEntityExist(clone) and IsEntityDead(clone) and existed then
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        if IsPedSprinting(clone) or IsPedRunning(clone) or IsPedWalking(clone) or IsPedRagdoll(clone) then
            if DoesEntityExist(clone) then
                DeleteEntity(peepee)
            end
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if existed and IsEntityDead(clone) then
            lantern = false
            cloneDying = true
            Citizen.Wait(3000)
            cloneDying = false
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        for key, value in pairs(Config.settings) do
            if IsEntityDead(clone) and existed then
                timerStarted = false
                timer = 0
                cloned = false
                dying = true
                ...
                if IsPedHuman(clone) then
                    ...
                    Citizen.Wait(200)
                    SetEntityVelocity(clone, 1.5, 1.5, 2.5)
                    RemoveBlip(blip)
                    existed = false
                    shareItem = false
                    if not value.narutoEffect then
                        if value.vfx then
                            local ex, ey, ez = table.unpack(GetEntityCoords(clone))
                            spawnExplode(0.3)
                            bloodExplode(clone)
                        else
                            PlaySoundFrontend("Core_Fill_Up", "Consumption_Sounds", true, 0)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsEntityPlayingAnim(PlayerPedId(), animation, "base", 3) and DoesEntityExist(clone) then
            ...
            if not IsPedSprinting(PlayerPedId()) or not IsPedRunning(PlayerPedId()) or not IsPedWalking(PlayerPedId()) or not IsPedRagdoll(PlayerPedId()) then
                Citizen.InvokeNative(0x5AD23D40115353AC, PlayerPedId(), clone, 2000, 1, 0, 0)
                Citizen.Wait(1000)
            end
        ...
        elseif IsEntityPlayingAnim(PlayerPedId(), animation, "base", 3) and not DoesEntityExist(clone) then 
...
function cloneCastSpell()
    ...
    TaskPlayAnim(clone, animation, "base", 8.0, -8.0, -1, 31, 0, true, 0, false, 0, false)
end
...
function cloneDrinkFirstaid()
    ...
    AttachEntityToEntity(medProp, clone, boneIndex, 0.06, -0.1, -0.11, -75.0, 0.00, -17.00, true, true, false, true, 1, true)
    TaskPlayAnim(clone, animation, "uncork", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
    Citizen.Wait(1000)
    cloneDrinking = true
    TaskPlayAnim(clone, animation, "chug_a", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
    Citizen.Wait(4500)
    SetEntityHealth(clone, cloneHealth+80, 1)
    Citizen.Wait(1080)
...
function getItemAnimation()
    local animation = "mech_inventory@item@fallbacks@tonic_potent@offhand"
    TaskPlayAnim(clone, animation, "use_quick", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
    Citizen.Wait(600)
end
...
function SET_BLIP_TYPE (clone)
	return Citizen.InvokeNative(0x23f74c2fda6e7c61, -1230993421, clone)
end
...
function cloneCrouch()
    TaskSetCrouchMovement(clone, true, 0, false)
end
...
Citizen.CreateThread(function()
    while true do
        ...
        local cHolding = Citizen.InvokeNative(0xD806CD2A4F2C2996, clone)
        if not IsPedSwimming(clone) and not Citizen.InvokeNative(0x3AA24CCC0D451379, clone) and not Citizen.InvokeNative(0xD453BB601D4A606E, clone) and not IsPedInWrithe(clone) and IsPedHuman(clone) and not Citizen.InvokeNative(0xA911EE21EDF69DAF, clone) and not cHolding and not Citizen.InvokeNative(0xF29A186ED428B552, clone, weapon) and not timerStarted and DoesEntityExist(clone) and IsPedStill(PlayerPedId()) and IsPedStill(clone) and not cloneControl and not IsPedRagdoll(clone) and not IsPedInMeleeCombat(clone) and cloneHealth >= 46 and hour >= 7 and hour <= 19 then            
            playCloneIdleAnim()
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        if DoesEntityExist(clone) then
            if GetEntityHealth(clone) >= 100 then
                SetEntityHealth(clone, 100, 0)
            else
                SetPlayerHealthRechargeMultiplier(clone, 0.0)
            end
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if DoesEntityExist(clone) then
            for key, value in pairs(Config.settings) do
                SetPedScale(clone, value.associateScale)                
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if not Citizen.InvokeNative(0x3AA24CCC0D451379, clone) and not Citizen.InvokeNative(0xD453BB601D4A606E, clone) and not IsPedInWrithe(clone) and IsPedHuman(clone) and not Citizen.InvokeNative(0xA911EE21EDF69DAF, clone) and not cHolding and not Citizen.InvokeNative(0xF29A186ED428B552, clone, weapon) and not timerStarted and IsPedRagdoll(PlayerPedId()) and not IsPedRagdoll(clone) and IsPedStill(clone) and not cloneControl and not cloneReacting and not cloneDrinking and cloneHealth >= 46 and hour >= 7 and hour <= 19 then
            Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 2, true)
            timer = 0
            timerStarted = false
            cloneLaughing = true
            cloneReacting = false
            if not IsPedOnMount(clone) or not IsPedInAnyVehicle(clone, false) then
                makeEntityFaceEntity(clone, PlayerPedId())
            end
            if not IsPedRagdoll(clone) then
                if reactionRandom == 0 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0x0CF840A9, 1, 1, 0, 0)
                elseif reactionRandom == 1 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0x0CF840A9, 1, 1, 0, 0)
                elseif reactionRandom == 2 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -402959, 1, 1, 0, 0)
                elseif reactionRandom == 3 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 803206066, 1, 1, 0, 0)
                elseif reactionRandom == 4 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 445839715, 1, 1, 0, 0)
                elseif reactionRandom == 5 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 1825177171, 1, 1, 0, 0)
                else
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -221241824, 1, 1, 0, 0)
                end
            end
        elseif not Citizen.InvokeNative(0x3AA24CCC0D451379, clone) and not Citizen.InvokeNative(0xD453BB601D4A606E, clone) and IsPedHuman(clone) and not Citizen.InvokeNative(0xF29A186ED428B552, clone, weapon) and not timerStarted and IsPedRagdoll(clone) and not cloneControl and not cloneReacting and not cloneDrinking and cloneHealth >= 46 and hour >= 7 and hour <= 19 then
            Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 2, true)
            timer = 0
            timerStarted = false
            Citizen.Wait(2000)
            ...
            if not IsPedRagdoll(clone) then
                if reactionRandom == 0 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 987239450, 1, 1, 0, 0)
                elseif reactionRandom == 1 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -553424129, 1, 1, 0, 0)
                elseif reactionRandom == 2 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 969312568, 1, 1, 0, 0)
                elseif reactionRandom == 3 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 354512356, 1, 1, 0, 0)
                elseif reactionRandom == 4 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 81347960, 1, 1, 0, 0)
                elseif reactionRandom == 5 then
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 1802342943, 1, 1, 0, 0)
                else
                    Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 486225122, 1, 1, 0, 0)
                end
            end
        end
        cloneLaughing = false
...
function playCloneIdleAnim()
    ...
    if not timerStarted then
        if IsEntityInWater(clone) and not IsPedSwimming(clone) and not IsPedOnMount(clone) then
    ...
    if not timerStarted and not cloneControl and not cloneLaughing and not IsPedSprinting(clone) or not IsPedRunning(clone) or not IsPedWalking(clone) and not IsPedRagdoll(clone) and not cloneDrinking and not cloneHealth <= 45 and not IsPedInMeleeCombat(PlayerPedId()) or not IsPedInMeleeCombat(clone) then
        timer = 0
        playingAnim = true
        timerStarted = false
        DeleteObject(offeredProp)
        ...
        if animRandom == 0 then    
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0x8B7F8EEB, 1, 1, 0, 0)
            Citizen.Wait(12000)
            ClearPedTasks(clone)
        elseif animRandom == 1 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0x81615BA3, 1, 1, 0, 0)
            Citizen.Wait(12000)
            ClearPedTasks(clone)
        elseif animRandom == 2 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0xBA51B111, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 3 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 0x43F71CA8, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 4 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 1256841324, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 5 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 1939251934, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 6 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 796723886, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 7 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -2106738342, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 8 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 935157006, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 9 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 901097731, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 10 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 831975651, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 11 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, -1118911493, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        elseif animRandom == 12 then
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 246916594, 1, 1, 0, 0)
            Citizen.Wait(8000)
            ClearPedTasks(clone)
        ...
        elseif animRandom == 14 then
            ...
            TaskPlayAnim(clone, canteenAnimation, "use_quick", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
            Citizen.Wait(700)
            carryCloneCanteen()
            AttachEntityToEntity(prop, clone, boneIndex, 0.12, 0.03, -0.07, -65.0, 0.00, -17.00, true, true, false, true, 1, true)
            Citizen.Wait(2000)
            AttachEntityToEntity(prop, clone, boneIndex, 0.09, 0.06, -0.05, -65.0, 0.00, -17.00, true, true, false, true, 1, true)            
            if IsEntityAttachedToEntity(prop, clone) then
        ...
        elseif animRandom == 15 then
            ...
            AttachEntityToEntity(prop, clone, boneIndex, 0.11, -0.03, -0.07, -90.0, 0.00, 0.00, true, true, false, true, 1, true)
            Citizen.Wait(600)
            TaskPlayAnim(clone, cannedAnimation, "right_hand", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
            Citizen.Wait(2900)
            DetachEntity(prop, 1, 1)
            Citizen.Wait(100)
            ClearPedTasks(clone)
        ...
        elseif animRandom == 16 then
            ...
            carryCloneProp(propName)
            AttachEntityToEntity(prop, clone, boneIndex, 0.09, -0.15, -0.10, -90.0, 190.00, 0.00, true, true, false, true, 1, true)
            Citizen.Wait(500)
            TaskPlayAnim(clone, foodAnimation, "quick_right_hand", 8.0, -1.0, 120000, 31, 0, true, 0, false, 0, false)
            Citizen.Wait(1500)
            DeleteObject(prop)
            Citizen.Wait(1000)
            ClearPedTasks(clone)
        elseif animRandom == 17 then
            makeEntityFaceEntity(clone, PlayerPedId())
            Citizen.InvokeNative(0xB31A277C1AC7B7FF, clone, 0, 0, 1927505461, 1, 1, 0, 0)
        elseif animRandom == 18 and IsPedStill(clone) then
            if not IsPedOnMount(clone) and not IsPedInAnyVehicle(clone, false) and not IsPedOnVehicle(clone, true) and not IsPedHangingOnToVehicle(clone) and not IsPedInFlyingVehicle(clone) and not IsPedSittingInAnyVehicle(clone) then
                TaskStartScenarioInPlace(clone, GetHashKey('WORLD_HUMAN_PEE'), -1, true, false, false, false)
                Wait(6000)
                CreatePeepee()
                Wait(22000)
                ClearPedTasks(clone)
        ...
        elseif animRandom == 19 and IsPedStill(clone) then
            if not IsPedOnMount(clone) and not IsPedInAnyVehicle(clone, false) and not IsPedOnVehicle(clone, true) and not IsPedHangingOnToVehicle(clone) and not IsPedInFlyingVehicle(clone) and not IsPedSittingInAnyVehicle(clone) then
                RequestAnimDict(drinkWaterAnimation)
                while ( not HasAnimDictLoaded(drinkWaterAnimation)) do 
                    Citizen.Wait( 100 )
                end
                TaskPlayAnim(clone, drinkWaterAnimation, "idle_f", 8.0, -8.0, -1, 0, 0, true, 0, false, 0, false)
                Wait(9000)
                ClearPedTasks(clone)
            end
        elseif animRandom == 20 and IsPedStill(clone) then  
            if not IsPedOnMount(clone) and not IsPedInAnyVehicle(clone, false) and not IsPedOnVehicle(clone, true) and not IsPedHangingOnToVehicle(clone) and not IsPedInFlyingVehicle(clone) and not IsPedSittingInAnyVehicle(clone) then
                TaskStartScenarioInPlace(clone, GetHashKey('WORLD_HUMAN_WASH_FACE_BUCKET_GROUND_NO_BUCKET'), -1, true, false, false, false)
                Wait(9000)
                Citizen.InvokeNative(0x523C79AEEFCC4A2A,clone,10, "ALL")
                Citizen.InvokeNative(0x6585D955A68452A5, clone)
                Citizen.InvokeNative(0x9C720776DAA43E7E, clone)
                Citizen.InvokeNative(0x8FE22675A5A45817, clone)
                ClearPedEnvDirt(clone)
                ClearPedBloodDamage(clone)
                ClearPedDamageDecalByZone(clone)
                ClearPedTasks(clone)
            end
        elseif animRandom == 21 and IsPedStill(clone) then  
            if not IsPedOnMount(clone) and not IsPedInAnyVehicle(clone, false) and not IsPedOnVehicle(clone, true) and not IsPedHangingOnToVehicle(clone) and not IsPedInFlyingVehicle(clone) and not IsPedSittingInAnyVehicle(clone) then
                RequestAnimDict(getWater1)
                while ( not HasAnimDictLoaded(getWater1)) do 
                    Citizen.Wait( 100 )
                end
                TaskPlayAnim(clone, getWater2, "use_quick", 8.0, -1.0, 120000, 23, 0, true, 0, false, 0, false)
                Citizen.Wait(1000)
                carryCloneCanteen()
                AttachEntityToEntity(prop, clone, boneIndex, 0.12, 0.03, -0.07, -65.0, 0.00, -17.00, true, true, false, true, 1, true)
                Citizen.Wait(300)
                TaskPlayAnim(clone, getWater1, "ground_far_swipe", 8.0, -1.0, 120000, 23, 0, true, 0, false, 0, false)
                Citizen.Wait(1400)
                if IsEntityAttachedToEntity(prop, clone) then
                    playCloneKeepkAnimation()
                    Citizen.Wait(1000)
...
function CreatePeepee() -- Create Peepee
    ...
    AttachEntityToEntity(peepee, clone, 1, 0.21, 0.13, -0.01, 2.0, 0.05, -35.21, false, true, false, false, 1, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5)
        local carrying = Citizen.InvokeNative(0xA911EE21EDF69DAF, clone)
        if IsPedHuman(clone) and DoesEntityExist(clone) and not Citizen.InvokeNative(0xA911EE21EDF69DAF, clone) and not cloneDrinking then

            
            for key, value in pairs(Config.settings) do
                if (cloned and value) or not carrying then
                    if IsPedSprinting(clone) or IsPedRunning(clone) then
                        Citizen.InvokeNative(0xF0A4F1BBF4FA7497, clone, -1, 0, 0)
                    end
                end
            end
            Citizen.InvokeNative(0xDF993EE5E90ABA25, clone, true)
            Wait(5000)
        end

        if IsPedRagdoll(clone) then
            SetPedToRagdoll(clone, 2000, 2000, 0, true, true, true)
        end
    end
end)
...
function spawnCompanion()
    ...
    clone = CreatePed(model, px, py, pz, GetEntityHeading(player), false, true) 
    SetEntityLoadCollisionFlag(clone, true)
    local coords = vector3(px+num, py+num, pz-0.5)
    SetEntityCoords(clone, coords)    
    playSmokeAudio()
    kagebunshin(0.8)
    local cx, cy, cz = table.unpack(GetEntityCoords(clone, 0))
    local blip = SET_BLIP_TYPE(clone) 
    SetEntityAlpha(clone, 120, false)
    SetBlipSprite(blip, sprite, 1)
    for key, value in pairs(Config.settings) do
        if value.displayNametag then
            SetPedPromptName(clone, value.nametag)
            SetPedNameDebug(clone, value.nametag)
            Citizen.InvokeNative(0x9CB1A1623062F402, blip, value.nametag)
        else
            SetPedPromptName(clone, companionName)
            SetPedNameDebug(clone, companionName)
            Citizen.InvokeNative(0x9CB1A1623062F402, blip, companionName)
        end
    end
    SetPedRandomComponentVariation(clone, 0)
    SetPedOutfitPreset(clone, 0, false)
    Citizen.InvokeNative(0x283978A15512B2FE, clone, true)
    SetBlockingOfNonTemporaryEvents(clone, false)
    SetPedAsGroupMember(clone, GetPedGroupIndex(PlayerPedId()))

    if not IsPedHuman(clone) then
        Citizen.InvokeNative(0x166E7CF68597D8B5, clone, 800)
        SetEntityHealth(clone, 800, 0)
        for key, value in pairs(Config.settings) do
            SetPedScale(clone, value.associateScale)
        end
    else
        SetEntityHealth(clone, GetEntityHealth(PlayerPedId()), 0)
    end

    if Citizen.InvokeNative(0xC346A546612C49A9, clone) then
        TaskFlyingCircle(clone, 1, 1, 1, 1, 1, 1)
    end
...
AddEventHandler('nic_clone:shadowClone', function()
    ...
    if target == nil or IsEntityDead(target) then
        for key, value in pairs(Config.settings) do
            ...
            if value.narutoEffect then
                clone = ClonePed(ped, true, true, true)
                SetEntityLoadCollisionFlag(clone, true)
                ...
                clone = ClonePed(ped, true, true, true)
                SetEntityLoadCollisionFlag(clone, true)
                local cPos = GetEntityForwardVector(clone)
                local coords = vector3(px, py, pz-1.0)
                SetEntityCoords(clone, coords)
                local cx, cy, cz = table.unpack(GetEntityCoords(clone, 0))

                Citizen.InvokeNative(0xAE99FB955581844A, ped, stagger, 0, 0, 0, 0, 0)
                Citizen.InvokeNative(0xAE99FB955581844A, clone, stagger, 0, 0, 0, 0, 0)

                -- SetEntityVelocity(ped, pos.x*multiplier, pos.y*multiplier, pos.z*multiplier)
                -- SetEntityVelocity(clone, pos.x*multiplier, pos.y*multiplier, pos.z*multiplier)
                ...
                if value.vfx then
                    ...
                    AddExplosion(px, py, pz-1.0, 12, 1.5, true, true, true)
            ...
            Citizen.InvokeNative(0xAE99FB955581844A, clone , hurtDuration, 0, 0, 0, 0, 0)
            local x, y, z = table.unpack(GetEntityCoords(clone, 0))
            local blip = SET_BLIP_TYPE(clone)
            SetBlipSprite(blip, sprite, 1)
            Citizen.InvokeNative(0x9CB1A1623062F402, blip, 'Clone')
            existed = true
            SetPedScale(clone, value.associateScale)
            SetEntityHealth(clone, GetEntityHealth(ped), 0)
            SetPedNameDebug(clone, name)
            SetPedPromptName(clone, name)
            SetBlockingOfNonTemporaryEvents(clone, false)
            SetPedAsGroupMember(clone, GetPedGroupIndex(ped))
            SetPedCanBeTargettedByPlayer(clone, PlayerId(), true)
            SetPedCanBeTargetted(clone, true)
            SetPedCanBeTargettedByTeam(clone, GetPedGroupIndex(ped), true)
...
function applyBloodDecals()    
    ...
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_Spray_Front_att", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_Spray_FRONT_V1", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Dead_John_bloody_back_vic", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Dead_John_bloody_chest_vic", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Mob4_Bitten_Leg_Blood_Soak_R", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_AnimalBlood_Lrg_Bloody", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_Spray_Right_V2", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_face_left", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_face_right", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_Soak_Right_Arm_Murder_for_Hire_Bump", 0.0, 0.0)
    Citizen.InvokeNative(0x46DF918788CB093F, clone, "PD_Blood_Soak_Left_Arm_Murder_for_Hire_Bump", 0.0, 0.0)
end
...
function spawnExplode(size)
    ...
    if not is_explode_effect_active then
        ...
        if Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_ptfx_dictionary)) then  -- HasNamedPtfxAssetLoaded
            ...
            current_explode_handle_id =  Citizen.InvokeNative(0xE6CFE43937061143,current_ptfx_name, clone, 0, 0, 0, 0, 0, 0, size, 0, 0, 0)    -- StartNetworkedParticleFxNonLoopedOnEntity
        else
            print("cant load ptfx dictionary!")
        end
    else
...
function kagebunshin(size)
    ...
    if not is_particle_effect_active then
        ...
        if Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_ptfx_dictionary)) then  -- HasNamedPtfxAssetLoaded
            ...
            current_ptfx_handle_id =  Citizen.InvokeNative(0xE6CFE43937061143,current_ptfx_name, clone, 0, 0, -0.2, 0, 0, 0, size, 0, 0, 0)    -- StartNetworkedParticleFxNonLoopedOnEntity
        else
            print("cant load ptfx dictionary!")
        end
    else
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        local hour = GetClockHours()
        local carryingLantern = Citizen.InvokeNative(0xF29A186ED428B552, clone, 0xF62FB3A3)
            ...
            if hour >= 7 and hour <= 19 then
                ...
                if not carryingLantern then
                    if IsPedInMeleeCombat(PlayerPedId()) then
                        ...
                        Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 1, true, false)
                    end
                end
            Citizen.InvokeNative(0x4899CB088EDF59B8, clone, 0xF62FB3A3, true, 0xF77DE93D)
            else
                if IsPedInMeleeCombat(clone) then
                    Citizen.InvokeNative(0xFCCC886EDE3C63EC, clone, 1, true)
                end
                weapon = 0xF62FB3A3
            end
...
AddEventHandler('nic_clone:cloneEquipWeapon', function(weapon)
    ...
    for key, value in pairs(Config.settings) do
        if IsPedHuman(clone) and value.carryRandomWeapon then 
            GiveWeaponToPed_2(clone, weapon, 10, true, true, 1, false, 0.5, 1.0, 1.0, true, 0, 0)
            SetCurrentPedWeapon(clone, weapon, true)
        end
    end
end)
        ...
        for key, value in pairs(Config.settings) do
            ...
            elseif value.cloneHealthDisplayType == "ring" then
                if not HasStreamedTextureDictLoaded("rpg_menu_item_effects") then
                    ...
                    elseif GetEntityHealth(clone) <= 90 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_8", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                    else
            ...
            elseif value.cloneHealthDisplayType == "core" then
                if not HasStreamedTextureDictLoaded("rpg_menu_item_effects") then
                    ...
                    elseif GetEntityHealth(clone) <= 55 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_1", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 60 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_2", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 65 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_3", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 60 then
                    ...
                    elseif GetEntityHealth(clone) <= 75 then
                        ...
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 70 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_6", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 85 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_7", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 80 then
                        DrawSprite("rpg_menu_item_effects", "rpg_tank_8", _x, _y+0.0125, 0.022, 0.036, 0.1, 255, 255, 255, 215, 0)
                        DrawSprite("blips_mp", "blip_health", _x, _y+0.0137, 0.017, 0.027, 0.1, 255, 255, 255, 215, 0)
                    elseif GetEntityHealth(clone) <= 95 then
