...
local blips = {
    { name = 'Bilihan ng Alaga', sprite = -1733535731, x = 2587.94, y = -1010.97, z = 43.25 }
}
...
local dogs = {
	...
	{
		['Text'] = "100 pesos - Chesbay Retriever",
		['SubText'] = "",
	...
	{
		['Text'] = "100 pesos - Labrador Retriever",
		['SubText'] = "",
		['Desc'] = "Level Required : ~pa~0",
		['Param'] = {
			['Price'] = 100,
	...
	{
		['Text'] = "100 pesos - Ches Bay Retriever",
		['SubText'] = "",
		['Desc'] = "Level Required : ~pa~0",
		['Param'] = {
			['Price'] = 15,
...
local function ShowNotification( _message )
	local timer = 200
	while timer > 0 do
		DisplayHelp(_message, 0.51, 0.86, 5000, 0.7, true, 161, 3, 0, 255, true, 10000)
		timer = timer - 1
		Citizen.Wait(0)
	end
end
...
Citizen.CreateThread( function()
	...
	repeat
		if WarMenu.IsMenuOpened('id_dog') then
			if WarMenu.Button("Ibenta ang Alaga") then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsZone then
			DisplayHelp(Config.Shoptext, 0.51, 0.90, 5000, 0.7, true, 255, 255, 255, 255, true, 10000)
...
AddEventHandler( 'bwrp:removedog', function (args)
	...
	ShowNotification( "Binenta mo ang iyong Alaga" )
...
AddEventHandler( 'bwrp:spawndog', function ( dog, skin, isInShop )
	...
	if isInShop then
		x, y, z, heading = 2583.89, -1008.86, 43.28, 276.31
	else
...
function SET_BLIP_SPRITE(animal)
    return Citizen.InvokeNative(0x74F74D3207ED525C, 305281166, true)
end
