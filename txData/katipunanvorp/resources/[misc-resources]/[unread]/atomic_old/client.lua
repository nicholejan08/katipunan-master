...
Citizen.CreateThread(function() 
	while true do 
		...
		if IsControlJustPressed(0, 0x446258B6) and not isDead then -- Delete key
    ...
    while true do
		...
		if WarMenu.IsMenuOpened('PlayerMenu') then
			...
			if WarMenu.Button('Kick player', "") then 
				...
				TriggerServerEvent("Log", systemlogs, "Moderator", GetPlayerName(PlayerId()).." Kicked player " .. GetPlayerName(player), 65535)
...
function Spectate()
	...
	TriggerServerEvent("Log", systemlogs, "Spectate", GetPlayerName(PlayerId()).." is spectating "..GetPlayerName(player), 65535)
end

function CancelCamera()
	TriggerServerEvent("Log", systemlogs, "Spectate", GetPlayerName(PlayerId()).." is no longer spectating "..GetPlayerName(player), 65535)
...
function Teleport()
	TriggerServerEvent("Log", systemlogs, "Teleport", GetPlayerName(PlayerId()).." teleported to " ..GetPlayerName(player), 65535)
...
function Message()
	...
	DisplayOnscreenKeyboard(1, "message", "", "", "", "", "", 175)
    ...
    if (GetOnscreenKeyboardResult()) then
		...
		TriggerServerEvent("Log", systemlogs, "Message", GetPlayerName(PlayerId()).." Sent message: " .. GetOnscreenKeyboardResult() .. ". Recipient: " .. GetPlayerName(player), 65535) -- Log example
...
function Bring()
	...
	TriggerServerEvent("Log", systemlogs, "Teleport", GetPlayerName(PlayerId()).." brought player " ..GetPlayerName(player), 65535)
end
...
TriggerEvent("chatL:addSuggestion", "/list", "Lists players on the server (Steam Name - Server ID")
RegisterCommand("list", function()
	local players = GetPlayers()
	
	for k, v in ipairs(players) do 
		print(GetPlayerName(v), GetPlayerServerId(v))
	end
end)
