...
RegisterNetEvent('redemrp_notification:stop')
AddEventHandler('redemrp_notification:stop',  function()
    hideUI()
end)
...
function bg(_timer)
    ...
    Citizen.CreateThread(function()
        ...
        while  show and timer > 0 do
            Citizen.Wait(0)
            DrawSprite("feeds", "toast_bg", 0.5, 0.15+offset, 0.25, height, 0.1, 000, 2, 2, 255, 1)
            DrawSprite("generic_textures", "hud_menu_4a", 0.5, 0.17+offset +(offset*2), loading, 0.01, 0.1, ProgressColor[1], ProgressColor[2], ProgressColor[3], 190, 0)
            timer = timer - 1
            loading = loading - del
        end
        hideUI()
