local keys = { ['O'] = 0xF1301666, ['S'] = 0xD27782E3, ['W'] = 0x8FD015D8, ['H'] = 0x24978A28, ['G'] = 0x5415BE48, ["ENTER"] = 0xC7B5340A, ['E'] = 0xDFF812F9, ['SPACE'] = 0xD9D0E1C0 }
...
local function ShowNotification( _message )
	local timer = 200
	while timer > 0 do
		DisplayHelp(_message, 0.51, 0.90, 0.3, 0.7, true, 161, 3, 0, 255, true, 10000)
		timer = timer - 1
		Citizen.Wait(0)
	end
end
	...
	while true do
		...
		for _,zone in pairs(Config.Marker) do
			if GetDistanceBetweenCoords(zone.x, zone.y, zone.z, GetEntityCoords(PlayerPedId()), false) < 2 then				                    
				DrawTxt("~COLOR_WHITE~Pindutin ang ~COLOR_WHITE~[~COLOR_RED~SPACE~COLOR_WHITE~] para magrenta ng Bangka", 0.51, 0.90, 0.3, 0.7, true, 255, 0, 0, 255, true)
